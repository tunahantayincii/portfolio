...
const FEEDBACK_LIMITS = { name: 80, email: 120, message: 600, projectTitle: 160 };
...
function cleanMessage(value) {
  return String(value || "").replace(/<[^>]*>/g, "").trim().slice(0, FEEDBACK_LIMITS.message);
}
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === "POST") {
    ...
    const projectTitle = clean(payload.projectTitle, FEEDBACK_LIMITS.projectTitle);
    const name = clean(payload.name, FEEDBACK_LIMITS.name);
    const email = clean(payload.email, FEEDBACK_LIMITS.email);
    const message = cleanMessage(payload.message);

    if (!projectId || !name || !message) return json(400, { error: "Ad ve geri bildirim alanı zorunlu." });
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json(400, { error: "E-posta adresi geçerli görünmüyor." });
    ...
    try {
      ...
      if (!project) return json(404, { error: "Proje bulunamadı. Sayfayı yenileyip tekrar deneyin." });
